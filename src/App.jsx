...
export default function BridgeExplorer() {
  ...
  const [homeTab,     setHomeTabRaw]    = useState(() => {
    ...
    return p === "pending" ? "pending" : p === "wallets" ? "wallets" : p === "failed" ? "failed" : "recent";
  });
  ...
  useEffect(() => {
    fetch("/api/stats")
      ...
      .catch(() => {});
    // Prefetch failed count for badge
    fetch("/api/failed?limit=1&offset=0")
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.total) setFailedTotal(d.total); })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    fetch("/api/pending")
      ...
      .then(async (d) => {
        ...
        // Verify ALL rows against the live API in parallel
        // Uses lightweight /api/status-check (no enrichment, just status) for speed
          ...
          rows.map(async (row) => {
            ...
            try {
              ...
              const res = await fetch(
                `/api/status-check?originChain=${row.originChain}&originTxHash=${row.originTx}`,
                { signal: controller.signal }
              );
              clearTimeout(timeout);
              if (!res.ok) return row; // API error, keep it as pending
              const live = await res.json();
              if (!live || live.error) return row; // API error, keep it
  ...
  // Fetch failed transactions when tab is selected
  const fetchFailed = (offset = 0) => {
    setFailedLoad(true);
    fetch(`/api/failed?limit=${FAILED_LIMIT}&offset=${offset}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (d?.rows) {
          setFailed(d.rows);
          setFailedTotal(d.total || 0);
          setFailedOffset(offset);
        }
        setFailedLoad(false);
      })
      .catch(() => setFailedLoad(false));
  };

  useEffect(() => {
    if (homeTab === "failed" && failed.length === 0) {
      fetchFailed(0);
    }
  }, [homeTab]);
        ...
        {!result && !error && !loading && !walletData && (
          <div style={{ animation:"fadeUp 0.4s ease-out 0.1s both" }}>
            ...
            <div style={{ display:"flex", gap:0, marginBottom:16, borderBottom:`1px solid ${C.border}` }}>
              ...
              >TOP WALLETS</button>
              <button
                onClick={() => setHomeTab("failed")}
                style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:10, letterSpacing:"0.08em", color: homeTab==="failed" ? C.red : C.textDim, background:"none", border:"none", borderBottom: homeTab==="failed" ? `2px solid ${C.red}` : "2px solid transparent", padding:"8px 16px", cursor:"pointer", fontWeight: homeTab==="failed" ? 700 : 400, display:"flex", alignItems:"center", gap:6 }}
              >FAILED {failedTotal > 0 && <span style={{ fontSize:9, color:C.red, background:C.red+"18", border:`1px solid ${C.red}35`, borderRadius:4, padding:"0px 6px" }}>{failedTotal}</span>}</button>
            </div>
            ...
            {homeTab === "failed" && (
              <div style={{ marginBottom:40 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                  <div style={{ fontSize:9, color:C.textDim, fontFamily:"'IBM Plex Mono', monospace", letterSpacing:"0.12em" }}>// FAILED BRIDGE TRANSACTIONS</div>
                  <span style={{ fontSize:9, color:C.textDim, fontFamily:"'IBM Plex Mono', monospace" }}>{failedTotal} total</span>
                </div>
                ...
                {failedLoad ? (
                  ...
                  <>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8, padding:"0 16px 8px", borderBottom:`1px solid ${C.border}`, marginBottom:4 }}>
                      ...
                      <span style={{ fontSize:9, color:C.textDim, fontFamily:"'IBM Plex Mono', monospace", letterSpacing:"0.1em", flexShrink:0, width:70 }}>REFUND</span>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                      {failed.map((row, i) => {
                        const srcChain = getChain(row.originChain);
                        const dstChain = getChain(row.destinationChain);
                        const vol = row.volumeUsd || row.sellUsd;
                        const volStr = vol ? (vol >= 1e6 ? "$"+(vol/1e6).toFixed(1)+"M" : vol >= 1e3 ? "$"+Math.round(vol/1e3).toLocaleString()+"K" : "$"+Math.round(vol).toLocaleString()) : "—";
                        const ts = row.timestamp ? new Date(row.timestamp + "Z") : null;
                        const timeStr = ts ? ts.toLocaleDateString("en-US", { month:"short", day:"numeric" }) + ", " + ts.toLocaleTimeString("en-US", { hour:"2-digit", minute:"2-digit", timeZone:"UTC" }) + " UTC" : "";
                        const refundStr = row.refundStatus === "refund_confirmed" ? "✓ refunded" : row.refundStatus ? row.refundStatus.replace(/_/g," ") : "—";
                        const refundColor = row.refundStatus === "refund_confirmed" ? C.green : C.textDim;

                        return (
                          <div key={i}
                            style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"11px 16px", display:"flex", alignItems:"center", gap:8, transition:"border-color 0.15s", cursor:"pointer" }}
                            onMouseEnter={e => e.currentTarget.style.borderColor = C.red + "60"}
                            onMouseLeave={e => e.currentTarget.style.borderColor = C.border}
                            onClick={() => {
                              if (row.originTx && row.originChain) {
                                setInput(row.originTx);
                                window.history.pushState({}, "", `?chain=${row.originChain}&tx=${row.originTx}`);
                                doSearch(row.originChain, row.originTx);
                              }
                            }}
                          >
                            {/* Source */}
                            <div style={{ display:"flex", alignItems:"center", flexShrink:0 }}>
                              <div style={{ display:"flex", alignItems:"center", gap:6, width:90 }}>
                                {srcChain?.logo ? <img src={srcChain.logo} alt="" style={{ width:18, height:18, borderRadius:"50%", flexShrink:0 }} /> : <span style={{ width:18, height:18, borderRadius:"50%", background:C.surface2, flexShrink:0, display:"inline-block" }} />}
                                <span style={{ fontSize:11, fontFamily:"'IBM Plex Mono', monospace", color:C.textSub, fontWeight:600 }}>{srcChain?.short || row.originChain}</span>
                              </div>
                              <div style={{ display:"flex", alignItems:"center", gap:6, width:90 }}>
                                {dstChain?.logo ? <img src={dstChain.logo} alt="" style={{ width:18, height:18, borderRadius:"50%", flexShrink:0 }} /> : <span style={{ width:18, height:18, borderRadius:"50%", background:C.surface2, flexShrink:0, display:"inline-block" }} />}
                                <span style={{ fontSize:11, fontFamily:"'IBM Plex Mono', monospace", color:C.textSub, fontWeight:600 }}>{dstChain?.short || row.destinationChain}</span>
                              </div>
                            </div>

                            {/* Volume */}
                            <span style={{ fontSize:10, fontFamily:"'IBM Plex Mono', monospace", color:C.red, fontWeight:600, flexShrink:0, width:75 }}>{volStr}</span>

                            {/* Bridge */}
                            <div style={{ flexShrink:0, width:75 }}>
                              {row.bridge && <span style={{ fontSize:8, fontFamily:"'IBM Plex Mono', monospace", color:C.textDim, background:C.surface2, borderRadius:3, padding:"1px 5px", whiteSpace:"nowrap" }}>{row.bridge.replace(/_/g," ")}</span>}
                            </div>

                            {/* Refund */}
                            <div style={{ flexShrink:0, width:70 }}>
                              <span style={{ fontSize:8, fontFamily:"'IBM Plex Mono', monospace", color:refundColor, whiteSpace:"nowrap" }}>{refundStr}</span>
                            </div>
                            ...
                            <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                              ...
                              <span style={{ fontSize:9, color:C.textDim, padding:"2px 8px", border:`1px solid ${C.border}`, borderRadius:4, fontFamily:"'IBM Plex Mono', monospace" }}>trace</span>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    {/* Pagination */}
                    {failedTotal > FAILED_LIMIT && (
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12 }}>
                        <span style={{ fontSize:10, fontFamily:"'IBM Plex Mono', monospace", color:C.textDim }}>
                          {failedOffset + 1}–{Math.min(failedOffset + FAILED_LIMIT, failedTotal)} of {failedTotal}
                        </span>
                        <div style={{ display:"flex", gap:6 }}>
                          <button
                            disabled={failedOffset === 0}
                            onClick={() => fetchFailed(Math.max(0, failedOffset - FAILED_LIMIT))}
                            style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:9, color: failedOffset === 0 ? C.textDim : C.text, background:"none", border:`1px solid ${C.border}`, borderRadius:4, padding:"4px 12px", cursor: failedOffset === 0 ? "default" : "pointer", opacity: failedOffset === 0 ? 0.4 : 1 }}
                          >← prev</button>
                          <button
                            disabled={failedOffset + FAILED_LIMIT >= failedTotal}
                            onClick={() => fetchFailed(failedOffset + FAILED_LIMIT)}
                            style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:9, color: failedOffset + FAILED_LIMIT >= failedTotal ? C.textDim : C.text, background:"none", border:`1px solid ${C.border}`, borderRadius:4, padding:"4px 12px", cursor: failedOffset + FAILED_LIMIT >= failedTotal ? "default" : "pointer", opacity: failedOffset + FAILED_LIMIT >= failedTotal ? 0.4 : 1 }}
                          >next →</button>
                        </div>
                      </div>
                    )}
