...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Call API WITHOUT quoteId first (matches eng's approach, avoids stale quote routing)
    ...
    let upstream = await fetch(url.toString(), { headers: { "0x-api-key": apiKey } });
    let data = await upstream.json();

    // If the response is incomplete/pending AND we have a quoteId, retry with it
    // quoteId can improve response completeness for some bridge providers
    if (quoteId && data && !data.error && !data.name) {
      const isPending = data.status === "bridge_pending" || data.status === "origin_tx_confirmed" || data.status === "origin_tx_pending";
      const hasNoSteps = !data.steps || data.steps.length === 0;
      if (isPending || hasNoSteps) {
        const urlWithQuote = new URL("https://api.0x.org/cross-chain/status");
        urlWithQuote.searchParams.append("originChain", originChain);
        urlWithQuote.searchParams.append("originTxHash", originTxHash);
        urlWithQuote.searchParams.append("quoteId", quoteId);
        try {
          const retryRes = await fetch(urlWithQuote.toString(), { headers: { "0x-api-key": apiKey } });
          const retryData = await retryRes.json();
          if (retryData && !retryData.error && !retryData.name) {
            // Use retry data only if it has MORE info (more steps, or terminal status)
            const retryTerminal = retryData.status === "bridge_filled" || retryData.status === "bridge_failed" || retryData.status === "origin_tx_reverted";
            const retryHasSteps = retryData.steps && retryData.steps.length > 0;
            if (retryTerminal || (retryHasSteps && hasNoSteps)) {
              data = retryData;
            }
          }
        } catch { /* retry failed, use original response */ }
      }
    }
